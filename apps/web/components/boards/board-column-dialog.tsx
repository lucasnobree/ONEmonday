...
export function BoardColumnDialog({
  ...
  // column, so a stale edit never leaks between two columns.
  useEffect(() => {
    if (!open) return;
    setName(column?.name ?? "");
    setColor(column?.color ?? DEFAULT_COLOR);
    setWipEnabled(column?.wip_limit != null);
    setWipLimit(column?.wip_limit != null ? String(column.wip_limit) : "");
    setIsDoneColumn(column?.is_done_column ?? false);
    setError(null);
  }, [open, column]);
